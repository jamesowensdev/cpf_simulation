...
We define a triangular array of 3 camera traps and simulate an agent for 1,000 steps.
...
-   **Duration:** `r if(!is.null(sim_output$metadata$duration)) round(as.numeric(sim_output$metadata$duration), 4) else "N/A"` seconds
...
*(Note: The detection logic includes a 'Cooldown' period, preventing the counter from skyrocketing while the agent stands still.)*

![](simulation_gifs/README_Example_Run.gif)